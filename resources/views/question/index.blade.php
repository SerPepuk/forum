            ...
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        ...
                        <h2 class="col-md-2">
                            <span><a href="{{ route('question.create')}}" class="btn btn-primary">Задать вопрос</a></span>
                        </h2>
                    </div>
                            ...
                            <div class="row">
                                <div class="col-md-8">
                                    ...
                                    <select class="select2" multiple="multiple" name="tags[]" data-placeholder="Выберите теги" style="width: 100%;">
                                        @foreach($tags as $tag)
                                        <option value="{{$tag->id}}" {{isset(
                                            $_GET['tags']) && in_array($tag->id, $_GET['tags']
                                    ...
                                    <div class="row">
                                        <div class="col-md-4">
                                            ...
                                            <select name="time" class="form-control" id="time">
                                                <option {{isset($_GET['time']) && $_GET['time'] == 'all' ? 'selected' : null}} value="all">Всё время</option>
                                                <option {{isset($_GET['time']) && $_GET['time'] == 'today' ? 'selected' : null}} value="today">День</option>
                                                <option {{isset($_GET['time']) && $_GET['time'] == 'week' ? 'selected' : null}} value="week">Неделя</option>
                                                <option {{isset($_GET['time']) && $_GET['time'] == 'month' ? 'selected' : null}} value="month">Месяц</option>
                                            </select>
                                        </div>
                                        <div class="col-md-4">
                                            <label for="sort">Сортировка</label>
                                            <select name="sort" class="form-control" id="sort">
                                                <option {{isset($_GET['sort']) && $_GET['sort'] == 'old' ? 'selected' : null}} value="old">Сначала старые</option>
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <select name="answer" class="form-control" id="answer">
                                                <option {{isset($_GET['answer']) && $_GET['answer'] == 'all' ? 'selected' : null}} value="all">Любые</option>
                                                <option {{isset($_GET['answer']) && $_GET['answer'] == 'unanswered' ? 'selected' : null}} value="unanswered">Без ответа</option>
                                                <option {{isset($_GET['answer']) && $_GET['answer'] == 'answered' ? 'selected' : null}} value="answered">С ответом</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="row question">
                        ...
                        <div class="col-md-11">
                            ...
                            <div class="stats">
                                <div>
                                    <span>Создан:</span>
                                    <time itemprop="dateCreated" datetime="{{$question->created_at}}">{{\Carbon\Carbon::parse($question->created_at)->format('h:m d/m/Y')}}</time>
                                </div>
                                <div>
                                    @if($question->last_reply)
                                    <span class="">Последний ответ:</span>
                                    <span title="{{$question->last_reply}}">{{\Carbon\Carbon::parse($question->last_reply)->format('h:m d/m/Y')}}</span>
                                    @else
